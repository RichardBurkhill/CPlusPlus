/**
 * @file JMessage.hpp
 * @brief This file defines the base class and derived classes for parsing simulated Link 16 J-series messages.
 *
 * The structures are inspired by publicly known message types (e.g., J3.2 Air Track, J12 Target Assignment),
 * designed for educational use and portfolio demonstration. No classified or ITAR-restricted information was used.
 *
 * It also includes endian-safe utility functions for reading 16-bit and 32-bit integers from byte buffers.
 */
...
namespace Link16
{
    ...
    public:
        ...
        {
            if (size < 4)
                throw std::runtime_error("Buffer too small for JMessage header");
            message_type = readBE16(buffer);
            message_length = readBE16(buffer + 2);
            if (message_length * 2 > size)
                throw std::runtime_error("Buffer smaller than message length");
        }
    ...
    public:
        ...
        {
            if (size < 8)
                throw std::runtime_error("Buffer too small for J1 body");
    ...
    public:
        ...
        {
            if (size < 4)
                throw std::runtime_error("Buffer too small for J3 body");
    ...
    public:
        ...
        {
            if (size < 7)
                throw std::runtime_error("Buffer too small for J4 body");
    ...
    public:
        ...
        {
            if (size < 14)
                throw std::runtime_error("Buffer too small for J12 body");
    ...
    inline std::unique_ptr<JMessage> JMessage::createFromBuffer(const uint8_t *buffer, size_t size)
    {
        ...
        {
        case 1:
            msg = std::make_unique<J1Message>();
            break;
        case 3:
            msg = std::make_unique<J3Message>();
            break;
        case 4:
            msg = std::make_unique<J4Message>();
            break;
        case 12:
            msg = std::make_unique<J12Message>();
            break;
